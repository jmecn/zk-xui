...
public interface LayoutManager {
    ...
    /**
     * The method layouts a widget within a container, dependent of layout
     * constraints.
     * 
     ...
     * bound or not, and throw an exception by a try to bind it second time. This is
     * because the framework restricts, that a layout manager can be bound only to
    ...
    /**
     * The method is called by framework by unbinding the layout manager from its
     * container. The implementing class must register, whether this is already
     * bound or not, and throw an exception by a try to bind it second time. This is
     * because the framework restricts, that a layout manager can be bound only to
     * one container at a time.
