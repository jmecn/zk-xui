...
/**
 * This class implements a list widget to select and 'invoke' one ore more items. Listeners can be registered to track
 * selecting deselecting and 'invoking' of items.
 */
...
public class JCList<T> extends Widget implements IScrollable {
    ...
    public boolean isSelected(int pos) {
        return _selected.elementAt(pos);
    }
    ...
    private int getMaxLength() {
        int result = 0;
        for (int i = 0; i < _items.size(); i++) {
            String item = _items.elementAt(i).toString();
    ...
    private void printItem(int index, Rectangle rect) {
        ...
        String item = getItemRepresentation(_items.elementAt(index).toString());
