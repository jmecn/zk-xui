...
/**
 * This class implements a panel container for widgets. A panel is a rectangle with a specified color to place on it
 * other widgets. Each window has a panel called root panel that serves as root for the widget hierarchy.
 */
public class Panel extends WidgetContainer {
     ...
     * The constructor
     * 
     * @param width preferred width, if -1 is passed , no width is preferred, so the width of the panel depends only on
     *            the layout manager of the parent container or on the window width, if this is a root panel.
     * @param height preferred height, if -1 is passed , no height is preferred, so the width of the panel depends only
     *            on the layout manager of the parent container or on the window width, if this is a root panel.
     * @param height
     */
