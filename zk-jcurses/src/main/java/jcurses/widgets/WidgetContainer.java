...
public abstract class WidgetContainer extends Widget {
    ...
    /**
     * The method paits the container self, that is all except childrens. Is called
     * by <code>doPaint</code>. Must be overrided by derived classes.
     */
    ...
    private Rectangle getClippingRect(Rectangle rect, Widget widget) {
        ...
        Rectangle widgetRectangle = new Rectangle(widget.getAbsoluteX(), widget.getAbsoluteY(),
                widget.getSize().getWidth(), widget.getSize().getHeight());
     ...
     * constraints. This method is called by layout manager and cann't be called by
     * developer. To add a widget to the container, a developer must use methods of
     ...
     * layout manager and cann't be called by developer. To remove a widget from the
    ...
    /**
     * The method returns a list of widgets, that can handle shortcuts, within the
     * container.
     * 
     ...
     * container's children If null is returned, the entire container's surface is
     * used.
     */
